        ...
        <form action="<?php base_url("/editTicket/" . $ticket['ticket_id']) ?>" method="POST" class="formAdd p-2 pb-0">
            ...
            <div class="row ">
                <div class="form-group col-6 my-4 ">
                    ...
                    <textarea class="form-control" name="description" id="description" cols="30" rows="3"><?= $ticket['fault_description'] ?></textarea>
                </div>
                <div class="form-group col-6 my-4">
                    ...
                    <select class="form-control form-select" name="device" id="device">
                        ...
                        foreach ($device as $value) {
                            if ($ticket['device_type_id'] == $valueN) {
                                echo "<option value='" . $valueN . "'selected>" . $value . "</option>";
                            } else {
                ...
                <div class="form-group col-6 my-4 ">
                    ...
                    <input type="text" class="form-control" name="email" id="email"
                        value="<?= $ticket['email_person_center_g'] ?>">
                    <!--Correu per sessio-->
                </div>
                <div class="form-group col-6 my-4 ">
                    ...
                    <input type="text" class="form-control" name="name"
                        id="name"><?= $ticket['name_person_center_g'] ?></input>
                </div>
                <!--Professor-->
                <!--Fi variables-->
                <div class="col-12 bottom-center pe-0 ">
                    <button type="submit" class="btn btn-primary bold"><?= lang('ticketsLang.save') ?></button>
                    <a href="<?= base_url("/viewTickets") ?>"
                        class="btn btn-light btn-block"><?= lang('ticketsLang.cancel') ?></a>
                </div>
            </div>
        </form>
    </div>
